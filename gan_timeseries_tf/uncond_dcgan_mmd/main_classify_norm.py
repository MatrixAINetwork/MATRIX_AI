...
if __name__ == '__main__':
    ...
    out_fname = "uncond_dcgan_base_mmd_norm"
    out_dir = '/home/hfl/dataset/output/gan_timeseries_tf/{}'.format(out_fname)
    out_file = open("{}/classification_accuracy.md".format(out_dir), 'w')
    tb_head = ['dataset', 'knn', 'knn_gan', 'lsvc', 'lsvc_gan', 'lr', 'lr_gan']
    tb_head_str = md.table_row(tb_head)
    out_file.write(tb_head_str)
    tb_head_line_str = md.table_head_line(len(tb_head_str))
    out_file.write(tb_head_line_str)

    fname_list = [f for f in os.listdir(data_root) if os.path.isfile(f) is False]
    ...
    for fname in fname_list:
        ...
        # save result
        tb_row_elem = [fname,
                       acc_dic['knn'][1], acc_dic['knn_gan'][1],
                       acc_dic['lsvc'][1], acc_dic['lsvc_gan'][1],
                       acc_dic['lr'][1], acc_dic['lr_gan'][1]]
        tb_row_elem_str = md.table_row(tb_row_elem)
        out_file.write(tb_row_elem_str)
